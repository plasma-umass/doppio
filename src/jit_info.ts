/**
 * Contains opcode-specific JIT metadata.
 */
...
/**
 * Lookup table of opcode sizes, keyed on opcode.
 * If an entry is `0`, then the size is dynamically calculated.
 */
export const opcodeSize: {[opcode: number]: number} = function() {
  const rv = new Array(0xFF);
  const table: {[layout: number]: number} = {};
  const layoutType = enums.OpcodeLayoutType;
  const layouts = enums.OpcodeLayouts;
  const opcodes = enums.OpCode;
  ...
  layouts.forEach((type: enums.OpcodeLayoutType, opcode: enums.OpCode) => {
    rv[opcode] = table[type];
    // Special. These opcodes do not quite match their layouts.
    switch (opcode) {
      case opcodes.INVOKEINTERFACE:
      case opcodes.INVOKEINTERFACE_FAST:
        rv[opcode] = 5;
        break;
      case opcodes.TABLESWITCH:
      case opcodes.LOOKUPSWITCH:
      case opcodes.WIDE:
        // 0 means the size is context-sensitive.
        rv[opcode] = 0;
        break;
    }
  });
...
// Functions for calculating the size of opcodes with context-sensitive sizes.
export const contextualOpcodeSize: {[opcode: number]: (pc: number, code: Buffer) => number} = {};
contextualOpcodeSize[enums.OpCode.TABLESWITCH] = function(pc: number, code: Buffer): number {
    // Ignore padding bytes. The +1 is to skip the opcode byte.
  let paddedPC = pc + ((4 - (pc + 1) % 4) % 4) + 1;
  var defaultOffset = code.readInt32BE(paddedPC),
    low = code.readInt32BE(paddedPC + 4),
    high = code.readInt32BE(paddedPC + 8),
    entries = (high - low) + 1;
  return (paddedPC - pc) + 12 + (entries * 4)
}
contextualOpcodeSize[enums.OpCode.LOOKUPSWITCH] = function(pc: number, code: Buffer): number {
  // Skip padding bytes. The +1 is to skip the opcode byte.
  let paddedPC = pc + ((4 - (pc + 1) % 4) % 4) + 1;
  var defaultOffset = code.readInt32BE(paddedPC),
    nPairs = code.readInt32BE(paddedPC + 4);
  return (paddedPC - pc) + 8 + (8 * nPairs);
}
contextualOpcodeSize[enums.OpCode.WIDE] = function(pc: number, code: Buffer): number {
  switch (code.readUInt8(pc + 1)) {
    ...
    case enums.OpCode.IINC:
      return 6;
    default:
      throw new Error(`Invalid wide opcode.`);
  }
}

export function makeOnError(onErrorPushes: string[], pc: number) {
  return onErrorPushes.length > 0 ? `f.pc=${pc};f.opStack.pushAll(${onErrorPushes.join(',')});` : `f.pc=${pc};`;
}

const escapeStringRegEx = /\\/g;
/**
 * Table of factories for JIT information for opcodes with dynamic properties.
 */
export const dynamicOpJitInfo: {[opcode: number]: (m: Method, c: Buffer, pc: number) => JitInfo} = (() => {
  const rv: {[opcode: number]: (m: Method, c: Buffer, pc: number) => JitInfo} = {};
  const opcode = enums.OpCode;
  rv[opcode.INVOKEVIRTUAL_FAST] = rv[opcode.INVOKEINTERFACE_FAST] = makeInvokeVirtualJitInfo;
  rv[opcode.INVOKENONVIRTUAL_FAST] = makeInvokeNonVirtualJitInfo;
  rv[opcode.INVOKESTATIC_FAST] = makeInvokeStaticJitInfo;
  return rv;
})();


function makeInvokeStaticJitInfo(method: Method, code: Buffer, pc: number) : JitInfo {
  const index = code.readUInt16BE(pc + 1);
  const methodReference = <ConstantPool.MethodReference | ConstantPool.InterfaceMethodReference> method.cls.constantPool.get(index);
  ...
  return {hasBranch: true, pops: -paramSize, pushes: 0, emit: (pops, pushes, suffix, onSuccess) => {
    // () around ternary works around https://github.com/Microsoft/vscode/issues/6634
...
function makeInvokeVirtualJitInfo(method: Method, code: Buffer, pc: number) : JitInfo {
  const index = code.readUInt16BE(pc + 1);
  const methodReference = <ConstantPool.MethodReference | ConstantPool.InterfaceMethodReference> method.cls.constantPool.get(index);
  const paramSize = methodReference.paramWordSize;
...
function makeInvokeNonVirtualJitInfo(method: Method, code: Buffer, pc: number) : JitInfo {
  const index = code.readUInt16BE(pc + 1);
  const methodReference = <ConstantPool.MethodReference | ConstantPool.InterfaceMethodReference> method.cls.constantPool.get(index);
  const paramSize = methodReference.paramWordSize;
  return {hasBranch: true, pops: -(paramSize + 1), pushes: 0, emit: (pops, pushes, suffix, onSuccess, code, pc, onErrorPushes) => {
...
/**
 * Table of JIT information for opcodes with static properties.
 */
export const opJitInfo: JitInfo[] = function() {

// Intentionally indented higher: emitted code is shorter.

const table = new Array<JitInfo>(0xFF);
const OpCode = enums.OpCode;
...
function generateCmpInfo(cmpOp: string): JitInfo {
  return {hasBranch: true, pops: 2, pushes: 0, emit: (pops, pushes, suffix, onSuccess, code, pc, onErrorPushes) => {
    const offset = code.readInt16BE(pc + 1);
    const onError = makeOnError(onErrorPushes, pc + 3);
    return `${onError}if(${pops[1]}${cmpOp}${pops[0]}){f.pc=${pc+offset}}`;
  }};
}

const cmpeq: JitInfo = generateCmpInfo("===");

table[OpCode.IF_ICMPEQ] = cmpeq;
table[OpCode.IF_ACMPEQ] = cmpeq;

const cmpne: JitInfo = generateCmpInfo("!==");

table[OpCode.IF_ICMPNE] = cmpne;
table[OpCode.IF_ACMPNE] = cmpne;

table[OpCode.IF_ICMPGE] = generateCmpInfo(">=");

table[OpCode.IF_ICMPGT] = generateCmpInfo(">");

table[OpCode.IF_ICMPLE] = generateCmpInfo("<=");

table[OpCode.IF_ICMPLT] = generateCmpInfo("<");

function generateSingleOpCmpInfo(cmpOp: string): JitInfo {
  return {hasBranch: true, pops: 1, pushes: 0, emit: (pops, pushes, suffix, onSuccess, code, pc, onErrorPushes) => {
    const offset = code.readInt16BE(pc + 1);
    const onError = makeOnError(onErrorPushes, pc + 3);
    return `${onError}if(${pops[0]}${cmpOp}){f.pc=${pc+offset}}`;
  }};
}

table[OpCode.IFNULL] = generateSingleOpCmpInfo("==null");

table[OpCode.IFNONNULL] = generateSingleOpCmpInfo("!=null");

table[OpCode.IFEQ] = generateSingleOpCmpInfo("===0");

table[OpCode.IFNE] = generateSingleOpCmpInfo("!==0");

table[OpCode.IFGT] = generateSingleOpCmpInfo(">0");

table[OpCode.IFLT] = generateSingleOpCmpInfo("<0");

table[OpCode.IFGE] = generateSingleOpCmpInfo(">=0");

table[OpCode.IFLE] = generateSingleOpCmpInfo("<=0");
