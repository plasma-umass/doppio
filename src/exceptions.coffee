...
class root.JavaException
  ...
  method_catch_handler: (rs, method, top_of_stack) ->
    ...
    handler = _.find exception_handlers, (eh) ->
      ...
      eh.start_pc <= cf.pc < eh.end_pc and rs.get_loaded_class(c2t(eh.catch_type), null, true)? and
        (eh.catch_type == "<any>" or ecls.is_castable rs, rs.get_loaded_class(c2t(eh.catch_type)))
    if handler?
