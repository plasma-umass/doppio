...
class TSTemplate implements ITemplate {
  ...
  private staticHeaderSet: { [clsName: string]: boolean} = {};
  private classesSeen: string[] = [];
  private headerPath: string = path.resolve(argv.standard.directory, "JVMClasses.d.ts");
  private staticHeaderPath: string = path.resolve(argv.standard.directory, "JVMStatics.d.ts");
  private headerStream: NodeJS.WritableStream;
  private staticHeaderStream: NodeJS.WritableStream;
  ...
  constructor(outputPath: string, private interfacePath: string) {
    ...
    this.staticHeaderStream = fs.createWriteStream(this.staticHeaderPath);
    this.headersStart();
  ...
  public headersStart(): void {
    var startText = `// TypeScript declaration file for JVM types. Automatically generated by doppioh.
 ...
 */
declare module `;
    this.headerStream.write(`${startText} JVMClasses {\n`);
    this.staticHeaderStream.write(`import JVMClasses = require('./JVMClasses');
${startText} JVMStatics {\n`);
  }
  ...
  public fileStart(stream: NodeJS.WritableStream): void {
    ...
    stream.write(`import JVMClasses = require("./JVMClasses");
import JVMStatics = require("./JVMStatics");\n`);
    for (i = 0; i < files.length; i++) {
      file = files[i];
...
export = JVMStatics;\n`, () => {});
  ...
  private generateClassDefinition(desc: string): void {
    if (this.headerSet[desc] !== undefined || util.is_primitive_type(desc)) {
      ...
      this.staticHeaderSet[desc] = true;
  ...
  private _processHeader(cls: ClassData.ReferenceClassData): void {
      ...
      this.headerStream.write(`  export interface ${this.jvmtype2tstype(desc, false)} ${superClass !== null ? `extends ${this.jvmtype2tstype(superClass.name)}` : ''} {
${generateTypedFields(cls)}
${generateMethodSignatures(cls)}
  }\n`);
      this.staticHeaderStream.write(`  export interface ${this.jvmtype2tstype(desc, false)} {
